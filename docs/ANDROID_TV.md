...
Uitvoer: `release\ThuisHub-Android-1.2.4.apk`, plus `release\ThuisHub-Android-TV-1.2.4.apk` als identieke compatibiliteitsnaam. Dit is een lokaal debug-ondertekende APK voor sideloadtests, geen Play Store-release. Open de universele APK op het Android-apparaat of installeer met `adb install -r <apk>` wanneer USB/netwerkdebugging bewust is ingeschakeld.
