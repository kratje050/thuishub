...
export function resolveAppPaths(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    metadataCacheDir: path.join(dataDir, 'metadata-cache'),
    metadataImageCacheDir: path.join(dataDir, 'metadata-cache', 'images'),
    metadataResponseCacheDir: path.join(dataDir, 'metadata-cache', 'responses'),
