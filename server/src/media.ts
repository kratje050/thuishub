...
async function probe(filePath: string) {
  try {
    const { stdout } = await execFileAsync(ffprobePath, ['-v', 'quiet', '-print_format', 'json', '-show_format', '-show_streams', filePath], { maxBuffer: 4 * 1024 * 1024, windowsHide: true });
    const json = JSON.parse(stdout);
    const video = json.streams?.find((x: any) => x.codec_type === 'video');
    const audio = json.streams?.find((x: any) => x.codec_type === 'audio');
    return { duration: Number(json.format?.duration) || null, videoCodec: video?.codec_name || null, audioCodec: audio?.codec_name || null, width: video?.width || null, height: video?.height || null, colorTransfer: video?.color_transfer || null };
  } catch { return { duration: null, videoCodec: null, audioCodec: null, width: null, height: null, colorTransfer: null }; }
}
...
export async function scanLibrary() {
  ...
  try {
    ...
    const upsert = db.prepare(`INSERT INTO media_items(source_id, kind, title, sort_title, year, series_title, season, episode, file_path, subtitle_path, size, duration, video_codec, audio_codec, width, height, color_transfer)
      VALUES(@sourceId, @kind, @title, @sortTitle, @year, @seriesTitle, @season, @episode, @filePath, @subtitlePath, @size, @duration, @videoCodec, @audioCodec, @width, @height, @colorTransfer)
      ...
      size=excluded.size, duration=excluded.duration, video_codec=excluded.video_codec, audio_codec=excluded.audio_codec, width=excluded.width, height=excluded.height, color_transfer=excluded.color_transfer, updated_at=CURRENT_TIMESTAMP`);
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        const [stat, info] = await Promise.all([fs.promises.stat(job.file), probe(job.file)]);
