...
function Player({ item, settings, onClose, onProgress, onFinished }: { item: MediaItem; settings: Settings; onClose: () => void; onProgress: (id: number, progress: MediaItem['progress']) => void; onFinished: () => void }) {
  ...
  const report = useCallback((state: 'playing'|'paused'|'error' = 'playing') => {
    ...
    if (!video) return;
    const position = Math.max(0, (Number(video.currentTime) || 0) + playbackOffset.current);
    const duration = playbackOffset.current > 0 ? Math.max(0, Number(playbackSession.current?.duration) || 0) : Number.isFinite(video.duration) ? Math.max(0, Number(video.duration) || 0) : Math.max(0, Number(playbackSession.current?.duration) || 0);
  ...
  const stopSession = useCallback((reason: string) => {
    ...
    const position = Math.max(0, video ? (Number(video.currentTime) || 0) + playbackOffset.current : Number(current.position) || 0);
    const duration = playbackOffset.current > 0 ? Math.max(0, Number(current.duration) || 0) : Number.isFinite(video?.duration) ? Math.max(0, Number(video?.duration) || 0) : Math.max(0, Number(current.duration) || 0);
    reportQueue.current = reportQueue.current.then(async () => {
      try {
  ...
  useEffect(() => {
    ...
    playbackStartedAt.current=performance.now();playbackOffset.current=0;setFirstFrameMs(null);
    ...
    void createPlaybackSession(item.id,localBrowserDeviceId(),requestedPosition,quality).then(result=>{
      ...
      else {playbackOffset.current=requestedPosition;useHls(String(result.urls?.playback||''),String((result.decision as any)?.mode||''),()=>report('error'));}
    }).catch(caught=>{if(!cancelled){setError(errorMessage(caught,'De lokale video kon niet starten.'));if(sessionCreated)report('error');}});
    const restore = () => { if (playbackOffset.current===0&&requestedPosition > 0 && requestedPosition < video.duration * .92) video.currentTime = requestedPosition; };
    const timer = window.setInterval(() => { if (!video.paused && Date.now() - lastSaved.current > 8000) report('playing'); }, 3000);
    video.addEventListener('loadedmetadata', restore, { once: true });
    return () => { cancelled=true;window.clearInterval(timer);restartPosition.current={mediaId:item.id,position:Math.max(0,(Number(video.currentTime)||0)+playbackOffset.current)};stopSession('local-player-closed');hlsRef.current?.destroy();video.removeEventListener('loadedmetadata',restore);delete video.dataset.thuishubPlaybackSession;video.removeAttribute('src');video.load(); };
  }, [item.id,quality]);

  useEffect(()=>{void api<typeof markers>(`/media/${item.id}/markers`).then(setMarkers);},[item.id]);
  useEffect(()=>{const video=videoRef.current;if(!video)return;const track=()=>{const absoluteTime=video.currentTime+playbackOffset.current;setActiveMarker(markers.find(marker=>absoluteTime>=marker.startTime&&absoluteTime<marker.endTime)||null)};video.addEventListener('timeupdate',track);return()=>video.removeEventListener('timeupdate',track);},[markers]);
  ...
  return <div className="player-layer">
    ...
    {showTechnical&&playbackInfo&&<aside className="technical-playback"><header><strong>Technische informatie</strong><button onClick={()=>setShowTechnical(false)}>Sluiten</button></header><div className="technical-grid"><span>Methode<b>{playbackInfo.decision.label}</b></span><span>Reden<b>{playbackInfo.decision.reasons?.join(' · ')||'Geen transcode nodig'}</b></span><span>Container<b>{playbackInfo.technical.container} → {playbackInfo.decision.outputContainer}</b></span><span>Video<b>{playbackInfo.decision.copyVideo?'Copy':'Transcode'} · {playbackInfo.technical.videoCodec} → {playbackInfo.decision.outputVideoCodec}</b></span><span>Audio<b>{playbackInfo.decision.copyAudio?'Copy':'Transcode'} · {playbackInfo.technical.audioCodec} · {playbackInfo.technical.audioChannels||'?'} kanalen{playbackInfo.technical.atmos?' · Dolby Atmos':''}</b></span><span>Hardwaredecode<b>{playbackInfo.technical.hardwareDecodeActive?'Actief':'Niet actief'}</b></span><span>Hardware-encode<b>{playbackInfo.technical.hardwareEncodeActive?'Actief':'Niet actief'}</b></span><span>Tijd tot eerste beeld<b>{firstFrameMs===null?'Meten…':`${firstFrameMs} ms`}</b></span><span>Beeld<b>{playbackInfo.technical.width}×{playbackInfo.technical.height} · {playbackInfo.technical.frameRate?.toFixed?.(3)||'?'} fps · {playbackInfo.technical.bitDepth}-bit</b></span><span>HDR<b>{playbackInfo.technical.hdr}{playbackInfo.technical.dolbyVisionProfile?` profiel ${playbackInfo.technical.dolbyVisionProfile}`:''}</b></span><span>Netwerk<b>{playbackInfo.decision.network}</b></span></div><div className="codec-badges">{[playbackInfo.technical.height>=2160?'4K':null,playbackInfo.technical.hdr!=='sdr'?String(playbackInfo.technical.hdr).toUpperCase():null,playbackInfo.technical.atmos?'Dolby Atmos':null,playbackInfo.decision.label].filter(Boolean).map((badge:string)=><b key={badge}>{badge}</b>)}</div></aside>}
    {activeMarker && ((activeMarker.type==='intro'&&settings.skipIntro)||(activeMarker.type==='credits'&&settings.skipCredits)||activeMarker.type==='commercial') && <button className="skip-button" onClick={()=>{if(videoRef.current)videoRef.current.currentTime=Math.max(0,activeMarker.endTime-playbackOffset.current);}}>{activeMarker.type==='intro'?'Intro overslaan':activeMarker.type==='credits'&&settings.skipCredits?'Aftiteling overslaan':'Reclame overslaan'} →</button>}
  </div>;
}
...
function TvNetworkSettingsPanel({settings,reload}:{settings:Settings;reload:()=>Promise<void>}){
  const[state,setState]=useState({localStreamingEnabled:settings.localStreamingEnabled,localStreamingAddress:settings.localStreamingAddress,localStreamingPort:settings.localStreamingPort,externalAccessMode:settings.externalAccessMode,externalAccessPort:settings.externalAccessPort,automaticDeviceDiscovery:settings.automaticDeviceDiscovery,dlnaDiscoveryEnabled:settings.dlnaDiscoveryEnabled,deviceRetentionDays:settings.deviceRetentionDays,castReceiverAppId:settings.castReceiverAppId,defaultQualityLan:settings.defaultQualityLan,defaultQualityTailscale:settings.defaultQualityTailscale,defaultQualityMobile:settings.defaultQualityMobile,defaultQualityDownload:settings.defaultQualityDownload,defaultQualityLiveTv:settings.defaultQualityLiveTv});
  const[addresses,setAddresses]=useState<string[]>([]);const[diagnostics,setDiagnostics]=useState<any>(null);const[external,setExternal]=useState<any>(null);const[busy,setBusy]=useState(false);const[notice,setNotice]=useState('');const[error,setError]=useState('');
  const[cast,setCast]=useState(describeCastEnvironment);
  const load=useCallback(async()=>{const[network,status,externalStatus]=await Promise.all([api<any>('/network/interfaces'),api<any>('/playback-devices/diagnostics'),api<any>('/external-access')]);setAddresses(network.addresses||[]);setDiagnostics(status);setExternal(externalStatus)},[]);
  ...
  const save=async()=>{if(state.externalAccessMode==='router'&&settings.externalAccessMode!=='router'&&!window.confirm('ThuisHub mag één aparte TCP-poort via UPnP of NAT-PMP op de router openen. Doorgaan?'))return;setBusy(true);setError('');setNotice('');try{await patch('/settings',{...state,externalAccessConsent:state.externalAccessMode==='router'});await reload();await load();setNotice('Netwerk- en tv-instellingen opgeslagen. Herstart ThuisHub wanneer een luisteradres of poort is gewijzigd.')}catch(caught:any){setError(caught.message)}finally{setBusy(false)}};
  ...
  return <section className="panel span-2 tv-network-settings"><div className="panel-heading"><div><h2>Netwerk en tv-streaming</h2><p>Apparaten worden automatisch gevonden; een tv-IP of Chromecast-ID invoeren is niet nodig.</p></div><span className="premium-badge">PRIVÉ-LAN</span></div>
    ...
    <div className="advanced-fields"><label>Externe toegang<select value={state.externalAccessMode} onChange={event=>setState({...state,externalAccessMode:event.target.value as Settings['externalAccessMode']})}><option value="home">Alleen thuisnetwerk</option><option value="router">Automatisch via router</option><option value="tailscale">Tailscale</option></select></label>{state.externalAccessMode==='router'&&<label>Aparte externe poort<input type="number" min="1024" max="65535" value={state.externalAccessPort} onChange={event=>setState({...state,externalAccessPort:Number(event.target.value)})}/></label>}<label>Status externe toegang<span>{state.externalAccessMode==='router'?(external?.router?.message||'Na opslaan controleren'):state.externalAccessMode==='tailscale'?(external?.tailscale?.message||'Tailscale controleren'):'Alleen thuisnetwerk'}</span></label></div>
