...
- Na een update verschijnt ThuisHub direct met een compact opstartvenster terwijl de server gereedkomt.
- De updatehelper en installer wachten niet langer meerdere vaste seconden voordat de nieuwe versie wordt gestart.
- Windows-updates worden sneller uitgepakt met behoud van dezelfde veilige controle en automatische herstart.

## Nieuw in 1.2.15

- Films hervatten direct op de juiste positie en het eerste bruikbare segment is aanzienlijk sneller beschikbaar.
- Audio-only transcoding laat de video ongemoeid en zet alleen kanalen of audiocodec om.
- NVIDIA hardwaredecode en -encode worden werkelijk gecontroleerd en zichtbaar gemaakt in Technische informatie.
- Externe toegang is kiesbaar: alleen thuisnetwerk, automatisch via UPnP/NAT-PMP of Tailscale.

## Nieuw in 1.2.14

- De Android-app vindt ThuisHub automatisch via mDNS, een snelle lokale netwerkuitzending of een begrensde directe subnetcontrole.
- Hoge lokale adressen zoals `.223` worden vroeg gecontroleerd, waardoor verbinden niet meer onnodig afbreekt.
- De Windows-updatehelper start de vernieuwde app expliciet en maakt het nieuwe venster zichtbaar.

## Nieuw in 1.2.13

- Automatisch installeren sluit ThuisHub af zonder de updatehelper mee te beëindigen.
- De installer vervangt de oude programmabestanden, controleert de nieuwe versie en start ThuisHub daarna opnieuw.
- Een apart installatielogboek maakt een eventuele Windows-installerfout zichtbaar.

## Nieuw in 1.2.12

- Films die transcoding nodig hebben starten sneller door segmenten van één seconde, geforceerde sleutelbeelden en het automatisch opruimen van oude tijdelijke segmenten.
- Bij lokaal afspelen gebruikt ThuisHub alleen de gewone videobediening en staat er geen grote dubbele balk meer over de film.
- Voor tv's en andere externe apparaten is de afstandsbediening compact, inklapbaar en beperkt tot functies die het gekozen apparaat werkelijk ondersteunt.
...
- DLNA-televisies krijgen bij transcoding nu een continue H.264/AAC-stereo MPEG-TS-stream in plaats van een HLS-afspeellijst.
- ThuisHub stuurt volledige DIDL-Lite-afspeelmetadata en probeert bij een Samsung HTTP 500 veilig opnieuw zonder metadata.
- Tijdzoeken via `TimeSeekRange.dlna.org` en begrensde LAN-toegang voor de nieuwe DLNA-stream zijn toegevoegd.

## Nieuw in 1.2.9

- De speler verbergt de voorbereidingsmelding nu zodra de video echt geladen is of speelt.
- Tijdens werkelijk bufferen of zoeken verschijnt de melding tijdelijk opnieuw en verdwijnt daarna automatisch.
- Transcode-HLS gebruikt kortere startsegmenten van 2 seconden in plaats van 6 seconden, zodat films eerder kunnen beginnen.

## Nieuw in 1.2.8

- De Android-app gebruikt na mDNS automatisch een tweede lokale detectiemethode op het eigen privé-wifisubnet.
- De fallback controleert uitsluitend de vaste ThuisHub-poort `8788`, stopt zodra de echte `/api/health`-identiteit is bevestigd en benadert geen internetadressen.
- Automatisch koppelen werkt daardoor ook op telefoons en routers die mDNS-advertenties niet goed doorgeven.

## Nieuw in 1.2.7

- Automatisch zoeken en verbinden staat bij een nieuwe Android-installatie standaard aan.
- De schakelaar onder **Verbindingsinstellingen** blijft leidend: wie automatisch verbinden uitzet, houdt die keuze.
- Voor lokale telefoon- en tv-detectie moet Windows het vertrouwde thuisnetwerk als **Privé** herkennen en gebruikt ThuisHub uitsluitend begrensde LAN-firewallregels.

## Nieuw in 1.2.6

- Windows, browser/server en Android controleren bij iedere start automatisch op de nieuwste stabiele GitHub-release.
- De controle draait op de achtergrond en blokkeert Home, bibliotheek, koppelen of afspelen niet.
- Android controleert ook wanneer nog geen pc-server is gekoppeld en toont alleen een melding wanneer werkelijk een nieuwere versie bestaat.
- Downloaden en installeren blijven handmatige, gecontroleerde acties; APK en Windows-installer worden vóór gebruik met SHA-256 geverifieerd.
...
- Onder **Verbindingsinstellingen** kan automatisch zoeken en verbinden aan of uit worden gezet; in 1.2.5 stond dit standaard uit.
...
- Portable: open `release\ThuisHub-Portable-1.2.16.exe`.
...
```powershell
.\scripts\generate-release-metadata.ps1 -Version 1.2.16
